...
SAL_SPI::SAL_SPI(spi_type *spi)
{
    ...
    spi_init_struct.mclk_freq_division = SPI_MCLK_DIV_8;//二分频
...
void SAL_SPI::begin()
{
    ...
    if (hspi == SPI1)
    {
        ...
        SAL_GPIO_Init(PA5, PIN_MUX, PULL_NONE); //SCL
        SAL_GPIO_Init(PA6, INPUT, PULL_NONE); //MISO
        SAL_GPIO_Init(PA7, PIN_MUX, PULL_NONE); //MOSI
    }
    else if (hspi == SPI2)
    {
        crm_periph_clock_enable(CRM_SPI2_PERIPH_CLOCK, TRUE);
        SAL_GPIO_Init(PB13, PIN_MUX, PULL_NONE); //SCL
        SAL_GPIO_Init(PB14, INPUT, PULL_NONE); //MISO
        SAL_GPIO_Init(PB15, PIN_MUX, PULL_NONE); //MOSI
    }
    /* 注意,SPI3使用了JT脚,使用前请警用,并且要处理无法下载的问题,如果出现无法现在,请按住复位键清除flash */
    #ifdef SPI3 
    else if(hspi == SPI3)
    {
        crm_periph_clock_enable(CRM_SPI3_PERIPH_CLOCK, TRUE);
        SAL_GPIO_Init(PB3, PIN_MUX, PULL_NONE); //SCL
        SAL_GPIO_Init(PB4, INPUT, PULL_NONE); //MISO
        SAL_GPIO_Init(PB5, PIN_MUX, PULL_NONE); //MOSI
    }
    #endif
    #ifdef SPI4 
    else if(hspi == SPI4)
    {
        ...
        SAL_GPIO_Init(PE2, PIN_MUX, PULL_NONE); //SCL
        SAL_GPIO_Init(PE5, INPUT, PULL_NONE); //MISO
        SAL_GPIO_Init(PE6, PIN_MUX, PULL_NONE); //MOSI
    }
    #endif
...
uint8_t SAL_SPI::transmit(uint8_t reg)
{
    spi_i2s_data_transmit(this->spi,reg);
    while(spi_i2s_flag_get(this->spi, SPI_I2S_TDBE_FLAG) == RESET);
    spi_i2s_data_transmit(this->spi, 0x000);//引导scl时钟产生
    while(spi_i2s_flag_get(this->spi, SPI_I2S_RDBF_FLAG) == RESET);
    /*REST表示寄存器空闲*/
    while (spi_i2s_flag_get(this->spi, SPI_I2S_BF_FLAG) == TRUE);
