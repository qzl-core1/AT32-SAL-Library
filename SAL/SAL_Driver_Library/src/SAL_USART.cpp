...
/*启动串口的函数*/
void SAL_USART::begin(uint32_t BaudRate,uint32_t preempt_priority, uint32_t sub_priority)
{
    ...
    if (_USARTX == USART1)
    {
        ...
        SAL_GPIO_Init(PA10, INPUT, PULL_UP);
        crm_periph_clock_enable(CRM_USART1_PERIPH_CLOCK, TRUE);
        nvic_irq_enable(USART1_IRQn, preempt_priority , sub_priority);
    }
    else if (_USARTX == USART2)
    {
        crm_periph_clock_enable(CRM_USART2_PERIPH_CLOCK, TRUE);
        ...
        /*PA3->RX*/
        SAL_GPIO_Init(PA3, INPUT, PULL_UP);
        nvic_irq_enable(USART2_IRQn, preempt_priority , 1);
    }
    else if (_USARTX == USART3)
    {
        crm_periph_clock_enable(CRM_USART3_PERIPH_CLOCK, TRUE);
        ...
        /*PA3->RX*/
        SAL_GPIO_Init(PB11, INPUT, PULL_UP);
        nvic_irq_enable(USART3_IRQn, preempt_priority , sub_priority);
    }
